...
import {
  ...
  FileEdit,
  ClipboardList,
  Wallet, // 👈 Added Wallet Icon for Payroll
} from "lucide-react";
...
const adminMenu = [
  ...
  {
    label: "Exam Builder",
    href: "/dashboard/recruitment/exams",
    icon: FileEdit,
  },
  {
    label: "Exam Results",
    href: "/dashboard/recruitment/results",
    icon: ClipboardList,
  },
  ...
  return (
    <aside className="w-full flex flex-col h-full bg-background border-r border-border transition-colors duration-300">
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
        ...
        {menu.map((item) => {
          // 🔴 FIX: Use startsWith to keep active state when inside subpages (e.g. Payroll Details)
          // Exception: Dashboard is exact match so it doesn't light up for everything
          const isActive =
            item.href === "/dashboard"
              ? pathname === "/dashboard"
