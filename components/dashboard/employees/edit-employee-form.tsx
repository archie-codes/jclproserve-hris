...
export function EditEmployeeForm({
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      allowance:
        employee.allowance || employee.taxableAllowance
          ? (employee.allowance || employee.taxableAllowance) / 100
          : 0,
    },
  });
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    setIsLoading(true);
    try {
      // 👇 NEW: Multiply back by 100 to safely store as cents in Neon DB
      const formattedData = {
        ...data,
        basicSalary: Math.round(data.basicSalary * 100),
        allowance: Math.round(data.allowance * 100),
      };

      // Pass the formattedData instead of raw data
      const result = await updateEmployee(employee.id, formattedData as any);
