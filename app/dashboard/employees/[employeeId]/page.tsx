...
export default async function EmployeeProfilePage({ params }: PageProps) {
  ...
  const formatCurrency = (amountInCents: number | null) => {
    ...
    return new Intl.NumberFormat("en-PH", {
      style: "currency",
      currency: "PHP",
    }).format(amountInCents / 100); // Convert cents to integer/decimal
  };
  ...
  return (
    <div className="min-h-screen bg-muted/40 p-4 md:p-6 space-y-6 md:space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          {/* Ensure child components handle the relational objects correctly if needed */}
          {/* <IdCardGenerator employee={employee} /> */}
          <PrintButton data={employee} />
          <EditProfileButton employee={employee} />
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="overflow-hidden border-t-4 border-t-primary shadow-sm bg-card">
            ...
            <CardContent className="relative pt-0 pb-8 text-center">
              ...
              {/* UPDATED: Access position.title via relation */}
              <p className="text-muted-foreground font-medium">
              ...
              <div className="mt-6 grid grid-cols-2 gap-4 text-left">
                <div className="p-3 bg-muted/50 rounded-lg border border-border">
                  ...
                  {/* UPDATED: Access department.name via relation */}
                  <p className="font-semibold text-sm truncate text-foreground">
                ...
                <CardContent>
                  <dl className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <DetailRow
                      ...
                      value={formatDate(employee.dateHired)}
                    />
                    {/* UPDATED: Use relations */}
                    <DetailRow label="Position" value={employee.position?.title} />
                ...
                <CardContent>
                  <dl className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    ...
                    <div className="p-4 bg-blue-50/50 dark:bg-blue-900/10 rounded-lg border border-blue-100 dark:border-blue-800/30">
                      ...
                      <dd className="text-xl font-bold text-foreground font-mono">
                        {/* UPDATED: Mapped to taxableAllowance per DB schema */}
                        {formatCurrency(employee.taxableAllowance)}
                      </dd>
                    </div>
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg border">
                      ...
                      <dd className="text-sm font-medium text-foreground">
                        {employee.salaryType === "MONTHLY" || employee.salaryType === "SEMI_MONTHLY"
                          ? "Fixed Monthly Rate"
                          : "Daily Rate (No Work No Pay)"}
                      </dd>
                    </div>
